...
def show_qr_code_result(decoded_info):
    ...
    # 创建返回按钮
    return_button = Button(qr_code_result_window, text="返回二维码识别界面", command=close_qr_code_window)
    return_button.pack()
...
def close_qr_code_window():
    global qr_code_window, camera_thread, stop_camera_thread

    # 停止摄像头线程
    if stop_camera_thread is not None:
